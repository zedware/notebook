...
static SYMBOL *a_binary_search(SYMBOL *sym, unsigned int n, const char *s, unsigned int len, unsigned int max_len)
{
    int low = 0;
    int high = n;
    ...
    {
        int mid = (low + high) / 2;
        int res = strncasecmp(s, sym[mid].name, len);
/*
        if (res == 0 && len != sym[mid].length)
            res = -1;
*/
...
#define symbols_max_len 29
#define sql_functions_max_len 29

static SYMBOL *get_hash_symbol(const char *s, unsigned int len, bool function)
{
    SYMBOL *sym = a_binary_search(symbols, array_elements(symbols), s, len, symbols_max_len);
    if (sym)
        return sym;
        
/*
    if (function)
        return binary_search(sql_functions, s, len, sql_functions_max_len);
*/      
    return NULL;
}
...
int main(void)
{
    SYMBOL *s = get_hash_symbol("=", 1, 0);
    if (s)
        printf("%s\n", s->name);

    s = get_hash_symbol("SELECT", 6, 0);
    if (s)
        printf("%s\n", s->name);
    return 0;
}
