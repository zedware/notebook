...
sub print_usage{
  ...
  print <<EOF;
==========================================================================================
Usage: orzdba <options>
   -H,--help           Print help
   -i,--interval       Time interval in seconds
   -C,--count          Number of counts
   ...
   -com                Print MySQL Status(Com_(select|insert|update|delete))
   -hit                Print InnoDB Hit%.
   -innodb_rows        Print InnoDB Rows status(Innodb_rows_(inserted/updated/deleted/read))
   -innodb_pages       Print InnoDB Buffer Pool Pages status(Innodb_buffer_pool_pages_(data/free/dirty/flushed))
   -innodb_data        Print InnoDB Data status(Innodb_data_(reads/writes/read/written))
   -innodb_log         Print InnoDB Log status(Innodb_os_log_(fsyncs/written))
   ...
   -T,--threads        Print Threads status(Threads_(running|connected|created|cached)
   -rt                 Print MySQL response time(us)
   -B,--bytes          Print Bytes received/send from/to MySQL(Bytes_(received|sent))
...
Example(s):
   shell> ./orzdba -h 127.0.0.1 -P 3306 -mysql
...
sub get_options{
  ...
  GetOptions(\%opt,
    ...
    # FIXME: macOS
    if ($Config{osname} eq 'darwin') {
      print $LOG_OUT RED(),"WARNING: iowait always display as zero.\n",RESET();
      print $LOG_OUT RED(),"WARNING: network bytes always in total.\n",RESET();
      print $LOG_OUT RED(),"WARNING: -d|--disk disabled.\n",RESET();
      $disk = 0;
    }
...
sub get_sysinfo {
  ...
  # 1) Linux
  #    $ cat /proc/loadavg 
  #    3.46 0.84 0.28 26/487 1526
  # 2) macOS
  #    $ sysctl -n vm.loadavg
  #    { 2.01 3.64 3.32 }
  # iostat, top also have the info.
  if ($load) {
    my @sys_load = (0, 0, 0);
    if ($Config{'osname'} eq 'darwin') {
      my $line = `sysctl -n vm.loadavg | sed -e 's/{\ //g'`;
  ...
  if ($cpu or $disk) {
    ...
    if ($Config{'osname'} eq 'darwin') {
      my $line = `top | head -4 | tail -1 | awk '{print \$3, \$5, \$7}' | sed -e 's/%//g'`;
      @sys_cpu2 = split(/\s+/, $line);
      $user_diff_1 = int($sys_cpu2[0]);
      $system_diff_1 = int($sys_cpu2[1]);
      $idle_diff_1 = int($sys_cpu2[2]);
    } elsif ($Config{'osname'} eq 'linux') {
  ...
  if ($swap) {
    ...
    if ($Config{'osname'} eq 'darwin') {
      my $line = `top | head -8 | tail -1 | awk '{print \$7, \$9}' | sed -e 's/([0-9]*)//g'`;
      my @swaps = split(/\s+/, $line);
      $swap2{"pswpin"} = $swaps[0];
      $swap2{"pswpout"} = $swaps[1];
    } elsif ($Config{'osname'} eq 'linux') { 
  ...
  if ($net) {
    ...
    if ($Config{'osname'} eq 'darwin') {
      my $line = `top | head -9 | tail -1 | awk '{print \$3, \$5}' | sed -e 's/\\/[0-9]*[A-Z]//g'`;
      my @net = split(/\s+/, $line);
      %net2 = (
        "recv" => $net[0],
        "send" => $net[1]
...
sub get_mysqlstat {
  if ($mysql) {
    ...
    if ($not_first) {
      ...
      # printf "select_diff: %d, interval: %d\n", $select_diff, $interval;
